...
export default async function PageAppearance({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  const pageDoc = await database.selectFrom("pages").selectAll()
    .where("id", "=", pageId).where("orgId", "=", session.orgId).executeTakeFirst();
  if (!pageDoc) notFound();
  ...
  const [draft, versions, groupDocs, componentDocs] = await Promise.all([
    ...
    database.selectFrom("pageDesignVersions").selectAll().where("pageId", "=", pageDoc.id)
      .orderBy("publishedAt", "desc").orderBy("id", "desc").limit(PAGE_DESIGN_VERSION_HISTORY_LIMIT).execute(),
  ...
  const initialDesign = draft
    ? (draft.design && typeof draft.design === "object" && "presentation" in draft.design
      ? statusPageDesignSchema.parse(draft.design)
      : designWithPagePresentation(statusPageDesignSchema.parse(draft.design), pageDoc))
    : publishedDesign;
  ...
  return (
    <DesignEditor
      page={{
        ...
        termsUrl: pageDoc.termsUrl,
        privacyUrl: pageDoc.privacyUrl,
      ...
      versions={versions.map((version) => {
        const parsedDesign = statusPageDesignSchema.parse(version.design);
        const design = version.design && typeof version.design === "object" && "presentation" in version.design
          ? parsedDesign
          : designWithPagePresentation(parsedDesign, pageDoc);
        return { version: version.version, templateKey: design.templateKey, savedAt: version.publishedAt.toISOString(), design };
      })}
