...
export default async function PageOverview({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  const action = draft ? finishPageSetup.bind(null, pageId) : setPagePublicVisibility.bind(null, pageId, !page.publicVisible);
  const canPublish = page.isHub || componentCount > 0;
  return <div className="space-y-6">
    ...
    <section className="flex flex-col gap-4 border border-[var(--line)] bg-[var(--surface)] p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-mono font-semibold">{draft ? "Publish page" : page.publicVisible ? "Hide page" : "Publish page"}</h2><p className="mt-1 text-sm text-[var(--fg-dim)]">{draft && !canPublish ? "Add at least one visible service before publishing." : draft ? "Publishing makes the page available at its public URL." : page.publicVisible ? "Hide the page temporarily without deleting it." : "Make the page available to visitors again."}</p></div><PlatformActionForm action={action} successMessage={page.publicVisible && !draft ? "Page hidden" : "Page published"}><PlatformSubmitButton disabled={draft && !canPublish} pendingLabel="Saving…" className="border border-[var(--cyan)]/40 px-4 py-2 text-sm font-semibold text-[var(--cyan)] disabled:opacity-50">{draft || !page.publicVisible ? "Publish page" : "Hide page"}</PlatformSubmitButton></PlatformActionForm></section>
