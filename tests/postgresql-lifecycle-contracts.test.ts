...
describe("PostgreSQL lifecycle contracts", () => {
  ...
  it("claims queue work with row locking and skip-locked semantics", () => {
    for (const file of ["worker/notifications.ts", "worker/exports.ts", "worker/audit-delivery.ts", "worker/platform-jobs.ts"]) {
  ...
  it("uses Graphile Worker for distributed scheduling and atomic queue wake-ups", () => {
    ...
    expect(worker).toContain("await run({"); expect(worker).toContain("parseCronItems");
    expect(worker).not.toContain("while (!state.stopping)");
    expect(jobs).toContain("graphile_worker.add_job"); expect(jobs).toContain("job_key_mode := 'replace'");
  });
  it("does not compile empty PostgreSQL IN lists", () => {
    const dashboard = source("app/admin/(protected)/page.tsx"); const publicData = source("lib/public-data.ts");
    expect(dashboard).toContain("if (!pageIds.length)"); expect(publicData).toContain("safeVisibleIds.length === 0");
    expect(publicData).toContain("scopedIds.length === 0"); expect(publicData).toContain('incidentQuery.where("pageWide", "=", true)');
  });
  it("uses foreign-key cascades for owned page data", () => {
    const migration = source("db/migrations/001_initial_postgresql.sql");
    for (const table of ["components", "incidents", "subscribers", "metrics", "monitors", "assets"]) {
  ...
  it("contains no legacy database driver references in runtime, scripts, or tests", () => {
    const legacyDriver = new RegExp(["mon", "go"].join(""), "i"); const legacyIdentifier = new RegExp(["Object", "Id"].join(""));
    const files = ["app", "components", "lib", "worker", "scripts", "tests"].flatMap(filesBelow).filter((file) => /\.(ts|tsx|js|mjs|cjs)$/.test(file));
    for (const file of files) { const contents = source(file); expect(legacyDriver.test(contents), file).toBe(false); expect(legacyIdentifier.test(contents), file).toBe(false); }
  });
});
